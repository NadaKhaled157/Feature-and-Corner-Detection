...
#///////////////////////////////////////////////////////////////////////////////////////////
        ...
        self.radioButton_ssd_sift.clicked.connect(self.on_radio_selected)
...
#//////////////////////////////////////////////MATCHING CODE////////////////////////////////////////////////////////////////////
  ...
  #hereeeeeee   
    ...
    def sift_matching_drawing(self, template, kp1, desc1, img, kp2, desc2,flag=True, ratio_thresh=0.75):
        # Start the timer
        ...
        if flag:
            # Draw keypoints for both images to confirm detection
            for kp in kp1:
                x, y = int(kp.pt[0]), int(kp.pt[1])
                cv2.circle(output_img, (x, y), 3, (0, 255, 0), 1, lineType=cv2.LINE_AA)
            for kp in kp2:
            ...
            for idx1, idx2, dist in matches:
                ...
                # print(f"Drawing match: ({x1}, {y1}) -> ({x2}, {y2}), distance: {dist}")  # Debug
                cv2.line(output_img, (x1, y1), (x2, y2), color, 1, lineType=cv2.LINE_AA)
                cv2.circle(output_img, (x1, y1), 3, color, -1, lineType=cv2.LINE_AA)
                cv2.circle(output_img, (x2, y2), 3, color, -1, lineType=cv2.LINE_AA)
    ...
    def on_radio_selected(self):
        if  self.RadioButton_Harris_2.isChecked():
            pass
               ...
               self.label_param_19.setText(f"         Lowe's ratio  : {slider_val/100:.2f}    ")
               if self.image is not  None:
                    if self.radioButton_ssd_sift.isChecked():
                        self.sift_matching_drawing(self.image, self.kp1,self.des1,self.template, self.kp2, self.des2,flag=False,ratio_thresh=slider_val/100 )
                    else :
